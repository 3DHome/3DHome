...
Vector3 Matrix3::operator* (const Vector3& rkPoint) const
{
    ...
    for (size_t iRow = 0; iRow < 3; iRow++)
    {
        kProd[iRow] = m[iRow][0] * rkPoint[0] +
                m[iRow][1] * rkPoint[1] + m[iRow][2] * rkPoint[2];
    }
    return kProd;
}
...
Vector3 operator* (const Vector3& rkPoint, const Matrix3& rkMatrix)
{
    ...
    for (size_t iRow = 0; iRow < 3; iRow++)
    {
        kProd[iRow] = rkPoint[0] * rkMatrix.m[0][iRow] +
...
bool Matrix3::Inverse (Matrix3& rkInverse, Real fTolerance) const
{
    ...
    Real fDet = m[0][0] * rkInverse[0][0] +
            m[0][1] * rkInverse[1][0]+ m[0][2] * rkInverse[2][0];
...
void Matrix3::SingularValueDecomposition (Matrix3& kL, Vector3& kS, Matrix3& kR) const
{
    ...
    for (unsigned int i = 0; i < ms_iSvdMaxIterations; i++)
    {
        ...
        if (bTest1)
        {
            ...
            {
                // 2 x 2 closed form factorization
...
void Matrix3::Orthonormalize ()
{
    ...
    // compute q1
    Real fDot0 = m[0][0] * m[0][1] + m[1][0] * m[1][1] +
             m[2][0] * m[2][1];
        ...
        else
        {
            ...
            if (m[0][0] >= m[1][1])
            {
                ...
                if (m[0][0] >= m[2][2])
                {
                    ...
                    rkAxis.x = 0.5f * Math::Sqrt(m[0][0] - m[1][1] - m[2][2] + 1.0f);
                ...
                else
                {
                    ...
                    rkAxis.z = 0.5f * Math::Sqrt(m[2][2] - m[0][0] - m[1][1] + 1.0f);
            ...
            else
            {
                ...
                if (m[1][1] >= m[2][2])
                {
                    ...
                    rkAxis.y = 0.5f * Math::Sqrt(m[1][1] - m[0][0] - m[2][2] + 1.0f);
...
bool Matrix3::QLAlgorithm (Real afDiag[3], Real afSubDiag[3])
{
    ...
    for (int i0 = 0; i0 < 3; i0++)
    {
        ...
        for (iIter = 0; iIter < iMaxIter; iIter++)
        {
            ...
            for (i1 = i0; i1 <= 1; i1++)
            {
                Real fSum = Math::Abs(afDiag[i1]) + Math::Abs(afDiag[i1 + 1]);
