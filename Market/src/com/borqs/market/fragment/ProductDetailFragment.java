...
public class ProductDetailFragment extends BasicFragment implements
    ...
    private MarketBillingResult mBillRes;
...
//        mPager.setPageMargin(20);
        ...
        bottomLayout = ButterKnife.findById(mConvertView, R.id.bottom_layout);
        bottom = ButterKnife.findById(mConvertView, R.id.bottom);
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(BOTTOM_LAYOUT_VISABLE, bottom.getVisibility()==View.VISIBLE ? true : false);
...
//                }
        ...
        if (mData.purchased || orderinfo != null) {
            return PRODUCT_STATUS_DOWNLOAD;
        } else {
...
//                                showIndeterminate(R.string.action_delete);
    ...
    public void onDestroyView() {
        QiupuHelper.unregisterDownloadListener(ProductDetailFragment.class.getName());
...
//            }, 500);
        ...
        mApiUtil.getProductDetail(mActivity, product_id, 0, supported_mod,
                new RequestListener() {
    ...
    private void purchase() {
        ...
        if (mData.isFree || mData.purchased || mBillRes != null) {
            synchronized (mLocked) {
                isLoading = true;
            }
            onPayed(mBillRes);
...
//                    showConfirmDialog();
                ...
                if (mData != null && "object".equals(mData.type)) {
                    showUpdateDialog();
                } else {
                    purchase();
                }
    ...
    public void updateView(int downLoadedBytes, int totalBytes, int status) {
        if (DownloadUtils.isDownloading(status)) {
            ...
            if (status == DownloadManager.STATUS_FAILED) {
                Message mds = mHandler.obtainMessage(DOWNLOAD_FAILED);
                mHandler.sendMessage(mds);
