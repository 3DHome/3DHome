    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mGestureInProgress) {
            if ((action == MotionEvent.ACTION_POINTER_1_DOWN || action == MotionEvent.ACTION_POINTER_2_DOWN)
                ...
                // Initiate sloppy gestures if we've moved outside of the slop
                // area.
                final float edgeSlop = mEdgeSlop;
                final float rightSlop = mRightSlopEdge;
                final float bottomSlop = mBottomSlopEdge;
            ...
            } else if ((action == MotionEvent.ACTION_POINTER_1_UP || action == MotionEvent.ACTION_POINTER_2_UP)
                    && mSloppyGesture) {
                // Set focus point to the remaining finger
                int id = (((action & MotionEvent.ACTION_POINTER_ID_MASK) >> MotionEvent.ACTION_POINTER_ID_SHIFT) == 0) ? 1
                        : 0;
